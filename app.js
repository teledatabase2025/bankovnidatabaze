(() => {
  ...
  function initUploadPage() {
    ...
    grid.innerHTML = people.map((person, index) => `
      <button class="upload-card ${loaded.has(person.id) ? 'is-loaded' : ''}" data-person="${person.id}" type="button">
        ...
        <div class="account-preview"><small>ČÍSLO ÚČTU</small><strong>${esc(person.account.number)}</strong><span>${esc(person.account.bank)}</span></div>
        <div class="upload-load-zone">
          <div class="upload-card-head"><span class="upload-state">${loaded.has(person.id) ? 'VÝPIS NAHRÁN' : 'KLIKNUTÍM NAHRÁT'}</span></div>
  ...
  function initAnalysisPage() {
    ...
    function readFilters() {
      return {
        from: $('#highDateFrom').value,
        to: $('#highDateTo').value,
        direction: $('#highDirection').value,
        rawQuery: $('#highSearch').value.trim(),
        query: $('#highSearch').value.trim().toLocaleLowerCase('cs-CZ')
      };
    }

    function filterProblem(filters) {
      if (!filters.from || !filters.to || filters.from > filters.to) {
        return 'Zvolené datum od nesmí být pozdější než datum do.';
      }
      if (filters.rawQuery.length > 0 && filters.rawQuery.length < 3) {
        return 'Pro vyhledávání podle jména, protistrany nebo údaje transakce zadejte alespoň tři znaky.';
      }
      return '';
    }
    ...
    function queryMatches(values, query) {
      ...
      return values.some(value => String(value || '').toLocaleLowerCase('cs-CZ').includes(query));
    }

    function transactionMatches(tx, filters, extraValues = []) {
      if (tx.date < filters.from || tx.date > filters.to) return false;
      if (filters.direction !== 'all' && tx.direction !== filters.direction) return false;
      return queryMatches([
        tx.counterparty, tx.counterpartyAccount, tx.counterpartyBank,
        tx.note, tx.type, tx.channel, tx.variableSymbol, tx.location,
        ...extraValues
      ], filters.query);
    }

    function filterDescription(filters) {
      const directionLabel = filters.direction === 'in' ? 'pouze příchozí' : filters.direction === 'out' ? 'pouze odchozí' : 'příchozí i odchozí';
      return `${formatDate(filters.from)}–${formatDate(filters.to)} · ${directionLabel}${filters.query.length >= 3 ? ` · hledaný výraz „${esc(filters.rawQuery)}“` : ''}`;
    }
    ...
    function renderHigh(personId) {
      ...
      const transactions = D.transactions[person.accountId]
        .filter(tx => tx.amount > threshold)
        .filter(tx => transactionMatches(tx, filters, [person.name, person.account.number]))
        .sort((a, b) => b.date.localeCompare(a.date) || b.amount - a.amount);
      const openingBalance = balanceAt(person.accountId, filters.from, false);
      const closingBalance = balanceAt(person.accountId, filters.to, true);
      ...
      results.innerHTML = `
        <header class="result-heading compact-result-heading high-result-heading">
          ...
          <p>Transakce nad ${money(threshold)} · ${filterDescription(filters)}</p>
    ...
    function renderMutual(firstId, secondId) {
      ...
      const filters = readFilters();
      const filtered = unique.filter(tx => {
        if (tx.date < filters.from || tx.date > filters.to) return false;
        if (filters.direction === 'in' && tx.to.id !== first.id) return false;
        if (filters.direction === 'out' && tx.from.id !== first.id) return false;
        return queryMatches([
          tx.note, tx.type, tx.variableSymbol,
          tx.from.name, tx.from.account.number,
          tx.to.name, tx.to.account.number
        ], filters.query);
      });
      const total = filtered.reduce((sum, tx) => sum + tx.amount, 0);
      results.innerHTML = resultHeading(
        'ANALÝZA VZÁJEMNÝCH TRANSAKCÍ', first, second,
        `Přímé platby mezi zvolenými účty · ${filterDescription(filters)}.`
      ) + (filtered.length ? `
        <div class="compact-summary three-columns">
          <div><span>Nalezeno převodů</span><strong>${filtered.length}</strong></div>
          <div><span>Celkový objem</span><strong>${money(total)}</strong></div>
          <div><span>Období</span><strong>${formatDate(filters.from)}–${formatDate(filters.to)}</strong></div>
        </div>
    ...
    function renderCommon(firstId, secondId) {
      ...
      const baseFilter = tx => tx.date >= filters.from && tx.date <= filters.to && (filters.direction === 'all' || tx.direction === filters.direction);
      const firstGroups = groupCounterparties(firstTx.filter(baseFilter), excluded);
      const secondGroups = groupCounterparties(secondTx.filter(baseFilter), excluded);
      ...
      }).filter(match => match.directions.length).filter(match => queryMatches([
        match.firstGroup.name, match.firstGroup.account, match.firstGroup.bank,
        match.secondGroup.name, match.secondGroup.account, match.secondGroup.bank,
        ...match.firstGroup.txs.flatMap(tx => [tx.note, tx.type, tx.variableSymbol]),
        ...match.secondGroup.txs.flatMap(tx => [tx.note, tx.type, tx.variableSymbol])
      ], filters.query)).sort((a, b) => {
      ...
      results.innerHTML = resultHeading(
        ...
        `Společné třetí strany nalezené v obou bankovních výpisech · ${filterDescription(filters)}.`
      ) + (matches.length ? `
        <div class="compact-summary three-columns">
    ...
    function renderLastAnalysis() {
      if (!lastAnalysis) return;
      if (lastAnalysis.type === 'high') renderHigh(lastAnalysis.personId);
      if (lastAnalysis.type === 'mutual') renderMutual(lastAnalysis.firstId, lastAnalysis.secondId);
      if (lastAnalysis.type === 'common') renderCommon(lastAnalysis.firstId, lastAnalysis.secondId);
    }
    ...
    $$('.run-analysis').forEach(button => button.addEventListener('click', () => {
      ...
      if (type === 'high') {
        lastAnalysis = { type, personId: $('#highPerson').value };
        animateAnalysis(renderLastAnalysis, 'high');
        return;
      }
      ...
      lastAnalysis = { type, firstId, secondId };
      animateAnalysis(renderLastAnalysis);
    }));

    let filterTimer;
    ['#highDateFrom', '#highDateTo', '#highDirection'].forEach(selector => {
      $(selector).addEventListener('change', () => {
        const problem = filterProblem(readFilters());
        if (!problem) renderLastAnalysis();
      });
    });
    $('#highSearch').addEventListener('input', () => {
      clearTimeout(filterTimer);
      filterTimer = setTimeout(() => {
        const filters = readFilters();
        if (filters.rawQuery.length === 0 || filters.rawQuery.length >= 3) renderLastAnalysis();
      }, 220);
    });
    $('#resetFilters').addEventListener('click', () => {
      $('#highDateFrom').value = D.meta.periodFrom;
      $('#highDateTo').value = D.meta.periodTo;
      $('#highDirection').value = 'all';
      $('#highSearch').value = '';
      renderLastAnalysis();
    });
  }
